...
+  (MTMathUILabel*) createMathLabel:(NSString*) latex withFont:(CGFloat)fontSize
{
    MTMathUILabel* label = [[MTMathUILabel alloc] init];
    label.labelMode = kMTMathUILabelModeText;
    label.fontSize = fontSize > 0 ? fontSize : 12;
//    [self setHeight:height forView:label];
    label.latex = [NSString stringWithFormat:@"%@",latex];
    return label;
}
@end
