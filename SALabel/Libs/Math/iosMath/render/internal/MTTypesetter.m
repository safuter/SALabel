...
NSUInteger getInterElementSpaceArrayIndexForType(MTMathAtomType type, BOOL row) {
    switch (type) {
        ...
        case KMTMathAtomOverset:
        case KMTMathAtomUnderset:
            return 7;
    ...
    for (MTMathAtom* atom in preprocessed) {
        switch (atom.type) {
            ...
            case KMTMathAtomOverset: {
                ...
                MTOverSet *overset = (MTOverSet *)atom;
                [self addInterElementSpace:prevNode currentType:atom.type];
                [self makeOverset:overset];
//                [_displayAtoms addObject:display];
//                _currentPosition.x += display.width;
                break;
            }
            ...
            case KMTMathAtomUnderset: {
                ...
                MTUnderSet *underset = (MTUnderSet *)atom;
                [self addInterElementSpace:prevNode currentType:atom.type];
                [self makeUnderset:underset];
                break;
            }
...
#pragma mark - Underset
- (void) makeUnderset:(MTUnderSet*) underset {
    double superScriptShiftUp = 0;
    double subscriptShiftDown = 0;
    
    MTMathListDisplay *subScript = [MTTypesetter createLineForMathList:underset.topspace font:_font style:self.scriptStyle cramped:self.superScriptCramped];
    subScript.type = kMTLinePositionSubscript;
    subscriptShiftDown = fmax(subscriptShiftDown, _styleFont.mathTable.subscriptShiftDown);
//    subScript.position = CGPointMake(_currentPosition.x, _currentPosition.y - subscriptShiftDown);
    [_displayAtoms addObject:subScript];
   
    MTMathListDisplay* topspaceDisplay = [MTTypesetter createLineForMathList:underset.bottomspace font:_font style:_style cramped:true];
    
//    subScript.position = CGPointMake(subScript.position.x + (topspaceDisplay.width - subScript.width) / 2, subScript.position.y);
//    subScript.position = CGPointMake(_currentPosition.x + (topspaceDisplay.width - subScript.width) / 2, _currentPosition.y - topspaceDisplay.descent - _styleFont.mathTable.underbarVerticalGap - subScript.ascent); // 参考自underline实现
    /* 考虑topdisplay和subscript谁更宽的问题，如果sub更宽，则这里的position变为负值，导致后面计算宽度适合，宽度会变小，会显示不全 */
    subScript.position = CGPointMake(MAX(_currentPosition.x + (topspaceDisplay.width - subScript.width) / 2, 0), _currentPosition.y - topspaceDisplay.descent - _styleFont.mathTable.underbarVerticalGap - subScript.ascent); // 参考自underline实现
    
//    -(innerListDisplay.descent + _styleFont.mathTable.underbarVerticalGap)
    topspaceDisplay.position = CGPointMake(_currentPosition.x, _currentPosition.y);
    [_displayAtoms addObject:topspaceDisplay];
    
    _currentPosition.x += MAX(subScript.width, topspaceDisplay.width);
}

#pragma mark - Overset
- (void) makeOverset:(MTOverSet*) overset {
    double superScriptShiftUp = 0;
    double subscriptShiftDown = 0;
    
    MTMathListDisplay* superScript = [MTTypesetter createLineForMathList:overset.topspace font:_font style:self.scriptStyle cramped:self.superScriptCramped];
    superScript.type = kMTLinePositionSuperscript;
    superScriptShiftUp = fmax(superScriptShiftUp, self.superScriptShiftUp);
    superScriptShiftUp = fmax(superScriptShiftUp, superScript.descent + _styleFont.mathTable.superscriptBottomMin);
    superScript.position = CGPointMake(_currentPosition.x, _currentPosition.y + superScriptShiftUp + 10);
    
    [_displayAtoms addObject:superScript];
    
    
    MTMathListDisplay* bottomDisplay = [MTTypesetter createLineForMathList:overset.bottomspace font:_font style:_style cramped:true];
    
    superScript.position = CGPointMake(MAX(superScript.position.x + (bottomDisplay.width - superScript.width) / 2, 0), bottomDisplay.ascent + _styleFont.mathTable.overbarVerticalGap); // 参考自overline实现
    
    bottomDisplay.position = CGPointMake(_currentPosition.x, _currentPosition.y);
    
    [_displayAtoms addObject:bottomDisplay];
    
    _currentPosition.x += MAX(superScript.width, bottomDisplay.width);
}
