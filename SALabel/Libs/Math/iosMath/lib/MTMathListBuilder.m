...
- (MTMathAtom*) atomForCommand:(NSString*) command
{
    ...
    if (accent) {
        ...
        MTOverSet *overset = [MTOverSet atomWithType:KMTMathAtomOverset value:@""];
        overset.topspace = [self buildInternal:true];
        overset.bottomspace = [self buildInternal:true];
        return overset;
    } else if ([command isEqualToString:@"underset"]) {
        MTUnderSet *underset = [MTUnderSet atomWithType:KMTMathAtomUnderset value:@""];
        underset.topspace = [self buildInternal:true];
        underset.bottomspace = [self buildInternal:true];
        return underset;
    } else {
...
- (MTMathAtom*) buildTable:(NSString*) env firstList:(MTMathList*) firstList row:(BOOL) isRow
{
    ...
    while (!_currentEnv.ended && [self hasCharacters]) {
        ...
        //// 处理table，第一个字符是控制字符的问题，l,m.r. 控制居中
        if ([_currentEnv.envName isEqualToString:@"array"]) {
            MTMathAtom *controlAlignAtom = list.atoms.firstObject;
            if ([controlAlignAtom.nucleus isEqualToString:@"l"] || [controlAlignAtom.nucleus isEqualToString:@"m"] || [controlAlignAtom.nucleus isEqualToString:@"r"]) {
                controlAlignAtom.nucleus = @"";
            }
        }
        ////////////////////////////////////////////////////////////
        if (!list) {
